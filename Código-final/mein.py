...
def random_combination(configuration:dict) -> str: 
...
def compareCapnoCap (listR: list, listU:list) -> tuple[int, bool]:
    ...
    for i in listR:
        if i in capU and i in noCapU:
            ...
            time.sleep(2)
            loading(f"\nAh dale, obvio, claro, claro (•ิ _•ิ )...")  
...
def game_start(configuration:dict):
    ...
    return name, start, org_chain

# Usuario ingresa cadena
def user_start(configuration:dict, start):
    game_cont = False
    penalty : int = 0
    score : int = 0
    ...
    return score, user_chain, score, l_original, l_user, game_cont

# Juego
def det_score(configuration:dict, user_chain, org_chain, score, l_original, l_user, win:list, lifes:int):
    ...
    if configuration.get("Lifes") in {3, 5, 10}:
        if configuration.get("Data") == "letras":
            ...
            if flagExist and flagIndex:
                ...
                win.append(False)
                lifes -= 1
    print(win, lifes, score)
    return win, lifes, score
...
if __name__ == "__main__":
    ...
    play_again = True
    while play_again:
        name, start, org_chain = game_start(configuration)
        loading("Tu puedes!!!")
        while lifes != 0:
            score, user_chain, score, l_original, l_user, first = user_start(configuration, start)
            if first == True:
                print("\n" + " ♥INICIA PARTIDA （*＾ワ＾*)♥ ".center(106, "~"))
                second = det_score(configuration, user_chain, org_chain, score, l_original, l_user, win, lifes)
                if all(second) == True:
                ...
                if dead == "si":
                    ...
                    lead = history(user_tries, name, score)
                    print(lead)
                    print("Vuelve pronto")
                    break
            else:
